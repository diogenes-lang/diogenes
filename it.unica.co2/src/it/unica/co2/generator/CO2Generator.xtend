...
class CO2Generator implements IGenerator {
	...
	override void doGenerate(Resource resource, IFileSystemAccess fsa) {
		...
		fsa.generateFile(outputFilename, compile(resource) )
	}
...
«««		   	ops «sessionNames.join(" ")» : -> SessionVariable [ctor] .
	...
	/*
	 * fix tell with anonymous contracts
	 */
	 ...
	 */
	def fixNames(Set<?> definitions) {
		for (e : definitions)
			fixName(e)
	}
	
	def dispatch fixName(ProcessDefinition process) {
		process.name = process.name ?: "PROC_"+PROCESS_NAME_COUNT++
	}
	
	def dispatch fixName(ContractDefinition contract) {
		contract.name = contract.name ?: "CONTR_"+CONTRACT_NAME_COUNT++
	}
	
	def dispatch fixName(Object process) {}
